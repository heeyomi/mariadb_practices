...
-- 예제 :
-- salaries 테이블에서 사번이 10060인 직원의 급여 평균과 총 합계를 출력하되 최저 임금을 받은 시기와 최대 임금을 받은 시기도 출력
-- 문법적으로 오류
-- 의미적으로 맞다.(where절 때문에)
select avg(salary) ,sum(salary), min(salary), max(salary) from salaries where emp_no = '10060';