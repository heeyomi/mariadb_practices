...
select concat(first_name, ' ', last_name) 이름, gender 성별, hire_date 입사일 from employees order by hire_date asc;


-- 문제3. (count)
-- 여직원과 남직원은 각 각 몇 명이나 있나요?
select count(*) 여직원 from employees where gender ='f';
select count(*) 남직원 from employees where gender = 'm';
...
-- 문제7.
-- 전체 부서를 출력하려고 합니다. 순서는 부서이름이 긴 순서대로 출력해 보세요. length(dept_name)
